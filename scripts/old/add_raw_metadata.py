...
df["wrist_cam_extrinsics"] = [[] for j in range(len(df))]
...
for file in raw_metadata_file_names:
    ...
    with open(fp,) as f:
        ...
        df.loc[index, "lab"] = metadata["lab"]
        df.loc[index, "date"] = metadata["date"]
        df.loc[index, "timestamp"] = metadata["timestamp"]
        df.loc[index, "scene_id"] = metadata["scene_id"]
        df.loc[index, "task"] = metadata["current_task"]
        df.loc[index, "building"] = metadata["building"]
        df.loc[index, "trajectory_length"] = metadata["trajectory_length"]
